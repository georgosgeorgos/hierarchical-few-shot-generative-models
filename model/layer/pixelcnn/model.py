...
class PixelCNNLayer_down(nn.Module):
    ...
    def forward(self, u, ul, u_list, ul_list, cond_blocks=None):
        for i in range(self.nr_resnet):
            ...
            ul = self.ul_stream[i](
                ul, a=torch.cat((u, ul_list.pop()), 1), cond_blocks=cond_blocks
            )
...
class PixelCNN(nn.Module):
    def __init__(
        ...
        if self.mode == "logistic_mix":
            ...
            self.nin_out = nin(nr_filters, nr_softmax_bins) # p
            #self.nin_out = nin(nr_filters, self.input_channels * nr_softmax_bins) # p
    ...
    def forward(self, x, cond_blocks=None, sample=False):
        ...
        ###      UP PASS    ###
        if x.size(0) != self.init_padding.size(0):
            _padding = self.init_padding[:x.size(0)]
            x = x if sample else torch.cat((x, _padding), 1) 
        else:
        ...
        for i in range(3):
            # resnet block
            u_out, ul_out = self.up_layers[i](
                u_list[-1], ul_list[-1], cond_blocks=cond_blocks
            )
            u_list += u_out
            ul_list += ul_out
